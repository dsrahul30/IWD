...
class parameters_initialization(object):
	...
	def __init__(self):
		...
		self.initialize_value_of_alpha()
	...
	def initialize_value_of_alpha(self, alpha = 0.1):
		"""
			If argument is supplied, the argument is returned.
			Otherwise, a default value is returned. This
			parameter is used only in MIWD
		"""
		self.parameter_list['alpha'] = alpha
		return alpha
...
def compute(all_parameters):
	...
	while iteration_count < maximum_iterations:
		...
		#===========================================================================================================
		"""
			For every N iterations, reinitialize soil on all nodes to Init Soil, except for T_TB.
			Reinitialize soil on T_TB to alpha*tau*init_soil.
		"""

		if iteration_count % 15 == 0 and iteration_count > 0:
			tau = random.random()
			soil = all_parameters['soil']
			prev = T_TB[len(T_TB)-1]
			for i in T_TB:
				now = i
				soil[prev][now] = soil[prev][now]*alpha*tau
				prev = now 
		#===========================================================================================================
